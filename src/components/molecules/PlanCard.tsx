...
export const PlanCard: React.FC<IPlanCardProps> = ({
  ...
  return (
    <div className="flex flex-col justify-between rounded-xl border border-brand-border bg-brand-card p-6 shadow-tesla transition-all duration-200 hover:border-white/30 hover:bg-brand-cardHover">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-md bg-[#18181b] border border-[#27272a] px-2.5 py-1 text-xs font-semibold text-gray-300 font-mono">
            {termDays} Days Term
          </span>
        </div>
      ...
      <div className="mt-6 border-t border-[#222226] pt-6 space-y-4">
        <div className="flex justify-between items-baseline">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Target APR</span>
          <span className="text-2xl font-extrabold tracking-tight text-white font-mono">{annualPercentageRate}</span>
        </div>
        ...
        <div className="space-y-1 text-xs text-gray-400 font-mono">
          ...
          <div className="flex justify-between text-brand-blue pt-1 border-t border-[#222226] font-semibold">
        ...
        {error && (
          <div className="rounded border border-red-500/40 bg-red-950/40 p-2.5 text-[11px] font-semibold text-red-400">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleAllocateClick} className="space-y-3 pt-2">
          <div>
            ...
            <input
              ...
              className="mt-1.5 w-full rounded-md border border-[#27272a] bg-[#18181b] px-3 py-2 text-sm font-mono text-white focus:border-white focus:outline-none focus:ring-1 focus:ring-white transition"
            />
          </div>

          <div className="flex justify-between items-center text-xs bg-[#18181b] p-3 rounded border border-[#27272a] font-mono">
            <span className="text-gray-400 font-sans font-medium text-[11px]">Est. Maturity Return:</span>
            <CurrencyDisplay amount={projectedReturn()} currency="USD" className="text-emerald-400 font-bold" />
          </div>

          <Button type="submit" variant="electric" size="md" className="w-full" isLoading={isAllocating}>
