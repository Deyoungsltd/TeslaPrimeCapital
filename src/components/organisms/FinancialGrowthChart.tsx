...
export const FinancialGrowthChart: React.FC<{ accruals: IAccrualChartItem[] }> = ({ accruals }) => {
  if (accruals.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-brand-border bg-brand-card/40 py-16 text-center">
        ...
        <h4 className="text-base font-bold text-gray-200 uppercase tracking-wider font-sans">No Yield Accruals Recorded Yet</h4>
  ...
  return (
    <div className="rounded-xl border border-brand-border bg-brand-card p-6 shadow-tesla space-y-6">
      <div className="flex flex-col justify-between gap-2 border-b border-[#222226] pb-5 sm:flex-row sm:items-center">
        <div>
          <h3 className="text-base font-extrabold text-white uppercase tracking-wider font-sans">Daily Compounding Accrual Trajectory</h3>
          <p className="text-xs text-gray-400 mt-0.5 font-mono">Enforcing approved policy: **Lump Sum at Plan Maturity**. Bars represent internally tracked daily growth.</p>
        </div>
      ...
      <div className="h-56 flex items-end justify-between gap-2 pt-6 pb-2 border-b border-[#222226] px-2 overflow-x-auto">
        {accruals.slice(0, 14).reverse().map((item, idx) => {
          ...
          return (
            <div key={idx} className="flex flex-col items-center flex-1 min-w-[36px] group relative">
              <div className="absolute -top-12 opacity-0 group-hover:opacity-100 transition pointer-events-none bg-black border border-white/30 text-white rounded py-1 px-2 text-[10px] font-mono shadow-2xl z-20 whitespace-nowrap">
                <div>{item.date}</div>
              ...
              <div
                ...
                className="w-full max-w-[28px] rounded-t bg-gradient-to-t from-emerald-950 via-emerald-600 to-emerald-400 transition-all duration-150 group-hover:brightness-125 shadow-md"
              />
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 pt-2 font-mono">
        ...
        <div className="rounded-lg bg-[#18181b] p-4 border border-[#27272a]">
          ...
          <span className="block mt-1 text-xs font-bold text-white truncate">{accruals[0]?.planName || 'Dynamic Growth'}</span>
        </div>
        <div className="rounded-lg bg-[#18181b] p-4 border border-[#27272a]">
          <span className="text-[10px] font-bold text-gray-400 uppercase">Maturity Disbursement</span>
          <span className="block mt-1 text-xs font-bold text-brand-blue uppercase">Lump Sum at Term End</span>
        </div>
      </div>
    </div>
  );
