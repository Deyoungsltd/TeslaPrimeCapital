...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside aria-label="Main Terminal Navigation" className="w-64 flex-shrink-0 border-r border-white/10 bg-black/95 p-5 transition-all duration-200 backdrop-blur-lg">
        ...
        {navItems.map((item) => {
          ...
          return (
            <a
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 rounded-xl px-4 py-3 text-xs font-bold tracking-wide transition-all duration-200 ${
                isActive
                  ? 'bg-brand-blue text-white shadow-blue-glow border border-white/20'
                  : 'text-gray-400 hover:bg-[#181822] hover:text-white border border-transparent'
              }`}
            >
              <svg className="h-4 w-4 flex-shrink-0 opacity-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a
              ...
              className={`flex items-center gap-3 rounded-xl px-4 py-3 text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                pathname?.startsWith('/admin')
                  ? 'bg-gradient-to-r from-brand-blue to-emerald-600 text-white shadow-blue-glow border border-white/30'
                  : 'text-gray-300 hover:bg-[#181822] hover:text-white border border-white/10'
              }`}
            >
