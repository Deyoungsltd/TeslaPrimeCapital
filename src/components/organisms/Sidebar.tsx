...
export const Sidebar: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'Structured Investments', href: '/dashboard/investments', icon: 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6' },
    ...
    { label: 'Support Desk', href: '/dashboard/support', icon: 'M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z' },
  ];
  ...
  return (
    <aside aria-label="Main Terminal Navigation" className="w-64 flex-shrink-0 border-r border-brand-border bg-brand-dark/90 p-4 transition-all">
      <nav className="space-y-1">
        {navItems.map((item) => {
          ...
          return (
            <a
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 rounded-md px-3 py-2.5 text-xs font-semibold tracking-wide transition-all ${
                isActive
        ...
        {isAdmin && (
          <div className="pt-6 mt-6 border-t border-gray-800">
            <span className="px-3 text-[10px] font-extrabold uppercase tracking-widest text-brand-gold/80">
              Executive Governance
            </span>
            <a
              href="/admin"
              className={`mt-2 flex items-center gap-3 rounded-md px-3 py-2.5 text-xs font-bold uppercase tracking-wider transition-all ${
                pathname?.startsWith('/admin')
                  ? 'bg-brand-blue/20 text-blue-400 border-l-2 border-brand-blue'
                  : 'text-gray-300 hover:bg-brand-blue/10 hover:text-blue-300'
              }`}
            >
