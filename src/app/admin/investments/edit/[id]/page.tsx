...
export default function AdminEditPlanPage() {
  ...
  const planIdOrSlug = (params?.id as string) || 'plan-bronze';
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (body.success && body.data) {
            const found = body.data.find((p: any) => p.planId === planIdOrSlug || p.id === planIdOrSlug) || body.data[0];
            if (found) {
              setPlanId(found.planId);
              setName(found.name);
              setDescription(found.description);
              setMinDepositUsd(found.minDepositUsd);
              setMaxDepositUsd(found.maxDepositUsd);
              setTermDays(found.termDays);
              setDailyRateNumeric(found.dailyRateNumeric);
              setAnnualPercentageRate(found.annualPercentageRate);
              setProfitText(found.profitText || '40% Profit');
              setImageUrl(found.imageUrl || '/branding/car-bronze.jpg');
              setFeaturesStr((found.features && Array.isArray(found.features) ? found.features.join(', ') : 'Portfolio Access, Investment Dashboard, Email Support'));
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setMsg(`Plan [${name}] updated cleanly! Car picture & features instantly synchronized across Landing Page and Investor Dashboard.`);
      } else {
  ...
  if (loading) {
    return (
      <div className="flex h-96 items-center justify-center font-mono text-xs text-gray-400 uppercase tracking-wider animate-pulse">
        <span>Loading Dedicated Plan Editor (`/admin/investments/edit/${planIdOrSlug}`)...</span>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 font-sans">
      <div className="flex items-center justify-between border-b border-[#20283E] pb-6">
        <div>
          <span className="text-[10px] font-bold uppercase tracking-widest text-brand-blue font-mono">Dedicated Edit Portal</span>
          <h1 className="text-2xl font-extrabold text-white sm:text-3xl mt-1 tracking-tight">Edit Plan &amp; Car Picture: {name}</h1>
        </div>
      ...
      <form onSubmit={handleUpdateSubmit} className="grid grid-cols-1 gap-8 md:grid-cols-2 items-start">
